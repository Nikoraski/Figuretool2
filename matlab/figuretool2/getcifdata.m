function cifdat = getcifdata(fid)
idx = 1;
cifStr = {};
while ~feof(fid)
...
for ii = 1:numel(cifStr)
    if any(ii==firstBL) BL = true; end
    if ~BL cifStr2{end+1} = cifStr{ii};
    else cifStr2{end} = [cifStr2{end},' ',cifStr{ii}(2:end)];
    end
    if any(ii==lastBL) BL = false; end
end
cifStr = cifStr2;
strout = {};
for ii = 1:numel(cifStr)
...
strout(emptyIdx) = [];
LP = false;
lpVar = cell(0,0); lpVal = cell(0,0);
cifdat.name = ''; cifdat.val  = '';
savedata = false;
for ii = 1:numel(strout) 
    strLine = strout{ii};
    switch strLine{1,1}
